...
async function takeAction(action, dataValue, waitForChange = true) {
    const privateKey = localStorage.getItem('ceeleos_key');
    var account = localStorage.getItem('ceeleos_account');
...
class ApiService {
    static getCurrentUser() {
        return new Promise((resolve, reject) => {
            if (!localStorage.getItem('ceeleos_account')) {
                return reject();
            }
            takeAction('login', { username: localStorage.getItem('ceeleos_account') }, false)
                .then(() => {
                    resolve(localStorage.getItem('ceeleos_account'));
                })
                .catch(err => {
                    localStorage.removeItem('ceeleos_account');
                    localStorage.removeItem('ceeleos_key');
                    reject(err);
                });
        });
    }
    ...
    static login({ username, key }) {
        return new Promise((resolve, reject) => {
            localStorage.setItem('ceeleos_account', username);
            localStorage.setItem('ceeleos_key', key);
    ...
    static startGame() {
        return takeAction('startgame', { username: localStorage.getItem('ceeleos_account') });
    }

    static playCard(cardIdx) {
        return takeAction('playcard', { username: localStorage.getItem('ceeleos_account'), player_card_idx: cardIdx });
    }

    static nextRound() {
        return takeAction('nextround', { username: localStorage.getItem('ceeleos_account') });
    }

    static endGame() {
        return takeAction('endgame', { username: localStorage.getItem('ceeleos_account') });
    }
